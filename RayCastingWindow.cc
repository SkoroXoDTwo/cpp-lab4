...
RayCastingWindow::RayCastingWindow(int width, int height) : Window(width, height) {
...
void RayCastingWindow::render() {
    ...
    SDL_SetRenderDrawColor(_renderer.get(), 115, 0, 0, 255);
    SDL_RenderFillRect(_renderer.get(), &r_sky);
    SDL_SetRenderDrawColor(_renderer.get(), 30, 30, 30, 255);
    ...
    double fieldOfView = pi / 3;

    double a = _player->dir();
    double sd = double(width()) / (2.0 * tan(fieldOfView / 2.0));
    double e = 0.00001;
